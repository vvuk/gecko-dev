                            ...
                            TiledLayerProperties* aLayerProperties /* = nullptr */)
{
  ...
  if (mPendingUpload) {
    ...
    // For a tiled buffer with double-buffered tiles, we hang onto the lock
    // until we stop compositing them. This is based on the assumption that
    // having double-buffered tiles means we're using tiles that don't have
    // an internal buffer, so they can't be drawn to on the client-side until
    // we stop compositing them.
    if (mOldTiledBuffer.HasDoubleBufferedTiles()) {
      mOldTiledBuffer.ReadUnlock();
    }
