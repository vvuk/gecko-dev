                          ...
                          const gfx::Rect& aVisibleRect)
{
  ...
  if (aEffectChain.mSecondaryEffects[EffectTypes::MASK]) {
    ...
    if (!srView) {
      ...
      gfxCriticalErrorOnce(gfxCriticalError::DefaultOptions(false)) << "Failed in DrawQuad 1 - Couldn't get ShaderResourceView!";
      return;
    }
  ...
  case EffectTypes::RENDER_TARGET:
    {
      ...
      if (!srView) {
        // XXX - There's a chance we won't be able to render anything, should we
        // just crash release builds?
        gfxCriticalErrorOnce(gfxCriticalError::DefaultOptions(false)) << "Failed in DrawQuad 2 - Couldn't get ShaderResourceView!";
        return;
      }
      mContext->PSSetShaderResources(0, 1, &srView);
  ...
  case EffectTypes::YCBCR: {
      ...
      if (!srViews[0] || !srViews[1] || !srViews[2]) {
        gfxCriticalErrorOnce(gfxCriticalError::DefaultOptions(false)) << "Failed in DrawQuad 345 - Couldn't get a ShaderResourceView!";
        return;
      }
  ...
  case EffectTypes::COMPONENT_ALPHA:
    {
      ...
      if (!srViews[0] || !srViews[1]) {
        gfxCriticalErrorOnce(gfxCriticalError::DefaultOptions(false)) << "Failed in DrawQuad 67 - Couldn't get a ShaderResourceView!";
        return;
      }
