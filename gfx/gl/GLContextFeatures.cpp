...
struct FeatureInfo
{
     ...
     */
    GLContext::GLExtensions mARBExtensionWithoutFunctionPrefix;
...
GLContext::IsFeatureProvidedByCoreSymbols(GLFeature feature)
{
    ...
    if (IsExtensionSupported(GetFeatureInfo(feature).mARBExtensionWithoutFunctionPrefix))
        return true;
...
GLContext::InitFeatures()
{
    for (size_t feature_index = 0; feature_index < size_t(GLFeature::EnumMax); feature_index++)
    {
        ...
        if (IsExtensionSupported(featureInfo.mARBExtensionWithoutFunctionPrefix))
        {
            mAvailableFeatures[feature_index] = true;
            continue;
        }
